...
class JackTokenizer3:
    ...
    def peek(self):#BROKEN
        print(len(self.input[self.currentLine])-1)
        print(self.spotLine)
    ...
    def numCombfPar(self):#NUMBER OF ARGUMENTS? (Current implementation for functions, add one for methods?)
        x=self.spotLine
        z=self.currentLine
        q=0
        while(self.input[z][x] !=")"):
            if self.input[z][x] == ",":
                q+=1
          
            x+=1
            if(self.input[z][x] =="(" and self.input[z][x+1] ==")"):
                return -1
        return q
